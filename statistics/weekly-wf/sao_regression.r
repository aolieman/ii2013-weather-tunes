...
wt.sample <- sao.wt[sample(1:nrow(sao.wt), 200, replace=FALSE),]
two_third_a4 <- 8.3 * 2/3 

# select weather features for regression
weather <- "hail + thunder + rain + fog + presh + presl + press + temph + templ + temp + dewh + dewl + dew + vish + visl + vis + windh + windl + wind + humh + huml + precip"
...
sink("regressions/sao_paulo/sao_regression_speechiness.out")
...
sink("regressions/sao_paulo/sao_regression_loudness.out")
